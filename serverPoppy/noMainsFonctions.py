...
SEUIL_TEMP = 54					# seuil de temperature pour alerter
...
#				position[position_counter]["45"]= Poppyboid.l_wrist_z.present_position
#				position[position_counter]["46"]= Poppyboid.l_wrist_x.present_position
				if position_counter!=1 and is_moving == False:
					...
					if abs(position[position_counter]["44"]-position[position_counter-1]["44"])>SEUIL_ANGLE:
						is_moving = True
#					if abs(position[position_counter]["45"]-position[position_counter-1]["45"])>SEUIL_ANGLE:
#						is_moving = True
#					if abs(position[position_counter]["46"]-position[position_counter-1]["46"])>SEUIL_ANGLE:
#						is_moving = True
...
#				position[position_counter]["55"]= Poppyboid.r_wrist_z.present_position
#				position[position_counter]["56"]= Poppyboid.r_wrist_x.present_position
				if position_counter!=1 and is_moving == False:
					...
					if abs(position[position_counter]["54"]-position[position_counter-1]["54"])>SEUIL_ANGLE:
						is_moving = True
#					if abs(position[position_counter]["55"]-position[position_counter-1]["55"])>SEUIL_ANGLE:
#						is_moving = True
#					if abs(position[position_counter]["56"]-position[position_counter-1]["56"])>SEUIL_ANGLE:
#						is_moving = True
...
#		position["45"]= Poppyboid.l_wrist_z.present_position
#		position["46"]= Poppyboid.l_wrist_x.present_position
...
#		position["55"]= Poppyboid.r_wrist_z.present_position
#		position["56"]= Poppyboid.r_wrist_x.present_position
		#export into a Json file ----------- Change file name if necessary
...
#				if abs(positionFin["55"]-positionActu["55"])>SEUIL_ANGLE :
#					if abs(positionFin["55"]-positionActu["55"])>LIMITE_ANGLE:
#						if positionFin["55"]-positionActu["55"]>0:
#							positionFin["55"]=positionActu["55"]+LIMITE_ANGLE
#						else:
#							positionFin["55"]=positionActu["55"]-LIMITE_ANGLE
#					self.robot.goto_position({'r_wrist_z': positionFin["55"]}, timeMultiplier*self.speed["bras_droit"], wait=False)
#				if abs(positionFin["56"]-positionActu["56"])>SEUIL_ANGLE :
#					if abs(positionFin["56"]-positionActu["56"])>LIMITE_ANGLE:
#						if positionFin["56"]-positionActu["56"]>0:
#							positionFin["56"]=positionActu["56"]+LIMITE_ANGLE
#						else:
#							positionFin["56"]=positionActu["56"]-LIMITE_ANGLE
#					self.robot.goto_position({'r_wrist_x': positionFin["56"]}, timeMultiplier*self.speed["bras_droit"], wait=False)
...
#				if abs(positionFin["45"]-positionActu["45"])>SEUIL_ANGLE :
#					if abs(positionFin["45"]-positionActu["45"])>LIMITE_ANGLE:
#						if positionFin["45"]-positionActu["45"]>0:
#							positionFin["45"]=positionActu["45"]+LIMITE_ANGLE
#						else:
#							positionFin["45"]=positionActu["45"]-LIMITE_ANGLE
#					self.robot.goto_position({'l_wrist_z': positionFin["45"]}, timeMultiplier*self.speed["bras_droit"], wait=False)
#				if abs(positionFin["46"]-positionActu["46"])>SEUIL_ANGLE :
#					if abs(positionFin["46"]-positionActu["46"])>LIMITE_ANGLE:
#						if positionFin["46"]-positionActu["46"]>0:
#							positionFin["46"]=positionActu["46"]+LIMITE_ANGLE
#						else:
#							positionFin["46"]=positionActu["46"]-LIMITE_ANGLE
#					self.robot.goto_position({'l_wrist_x': positionFin["46"]}, timeMultiplier*self.speed["bras_droit"], wait=False)
...
#				self.robot.goto_position({'r_wrist_z': self.robot.r_wrist_z.present_position}, t, wait=False)
#				self.robot.goto_position({'r_wrist_x': self.robot.r_wrist_x.present_position}, t, wait=False)
...
#				self.robot.goto_position({'l_wrist_z': self.robot.l_wrist_z.present_position}, t, wait=False)
#				self.robot.goto_position({'l_wrist_x': self.robot.l_wrist_x.present_position}, t, wait=False)
			if "tete" in SEMI_MOU :
	...
	def run(self):
		while True:
			try:
				...
				logger.exception("***** scanMotors error *****")
			time.sleep(5)
...
#			Poppyboid.l_wrist_z.compliant = True
#			Poppyboid.l_wrist_x.compliant = True
...
#			Poppyboid.r_wrist_z.compliant = True
#			Poppyboid.r_wrist_x.compliant = True
...
#			Poppyboid.l_wrist_z.torque_limit = torqueLimit
#			Poppyboid.l_wrist_z.compliant = False
#			Poppyboid.l_wrist_x.torque_limit = torqueLimit
#			Poppyboid.l_wrist_x.compliant = False
...
#			Poppyboid.r_wrist_z.torque_limit = torqueLimit
#			Poppyboid.r_wrist_z.compliant = False
#			Poppyboid.r_wrist_x.torque_limit = torqueLimit
#			Poppyboid.r_wrist_x.compliant = False
...
def reverse(moveName):
	...
	for nb_temps in range(moveData["nb_temps"]):
		if str(nb_temps+1) in moveData :
			...
			if '41' in moveData[str(nb_temps+1)] : 	#bras gauche
			...
			if '21' in moveData[str(nb_temps+1)] : 	#jambe droite
...
#					if v > temps_attente:
#						temps_attente = v
#					Poppyboid.goto_position({'r_wrist_z': positionFin["55"]}, v, wait=False)
#				if abs(positionFin["56"]-positionActu["56"])>seuil :
#					v = abs(positionFin["56"]-positionActu["56"])/vitesse
#					if v > temps_attente:
#						temps_attente = v
#					Poppyboid.goto_position({'r_wrist_x': positionFin["56"]}, v, wait=False)
...
#					if v > temps_attente:
#						temps_attente = v
#					Poppyboid.goto_position({'l_wrist_z': positionFin["45"]}, v, wait=False)
#				if abs(positionFin["46"]-positionActu["46"])>seuil :
#					v = abs(positionFin["46"]-positionActu["46"])/vitesse
#					if v > temps_attente:
#						temps_attente = v
#					Poppyboid.goto_position({'l_wrist_x': positionFin["46"]}, v, wait=False)
